# Plugins1/FinalMusic/fm_plugins/playback-controls/shuffle.py
...
@Client.on_message(filters.group & ~filters.bot, group=514)
async def shuffle_handler(client, m: Message):
    ...
    try:
        ...
        if not items or len(items) <= 1:
            return await m.reply(_lang.get("queue_empty", "قائمة الانتظار فارغة أو تحتوي على أغنية واحدة فقط"))
        current = items[0] if items else None
        remaining = items[1:] if len(items) > 1 else []
        if not remaining:
            return await m.reply(_lang.get("no_tracks_to_shuffle", "لا توجد أغانٍ في قائمة الانتظار للخلط"))
        ...
        await m.reply(_lang.get("shuffled", "تم خلط قائمة التشغيل بنجاح! ({} أغنية)").format(len(remaining)))
    finally:
        if old_bot_id:
