# Plugins1/FinalMusic/fm_plugins/playback-controls/pause.py
...
@Client.on_message(filters.group & ~filters.bot, group=511)
async def pause_handler(client, m: Message):
    ...
    try:
        ...
        if not playing_status or playing_status == "paused":
            try:
                return await m.reply(_lang.get("play_already_paused", "التشغيل متوقف بالفعل"))
            except:
                return
        await tune.pause(m.chat.id)
        try:
            sent = await m.reply(text=_lang.get("play_paused", "تم الإيقاف المؤقت").format(m.from_user.mention))
