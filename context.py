    ...
    async def wrapper(*args, **kwargs):
        ...
        for src in (aio_message, aio_callback, aio_event, aio_query):
            if src is not None and getattr(src, "from_user", None) is not None:
                event_from_user = src.from_user
                event_user_id = event_from_user.id
                break

        # تخزين انتهازي: أي مستخدم له يوزرنيم ويصل بحدث لأي بوت بالكلاستر
        # (رسالة/كولباك/جوين ريكوست...) يُحفَظ ربط يوزرنيمه بآيديه في خريطة
        # Redis مشتركة بين كل البوتات (helpers.redis.cache_username_id)،
        # فتتوفر لاحقاً لأوامر أي بوت آخر عبر aiogram مباشرة دون أي حاجة
        # لطلب Pyrogram من جديد لنفس المستخدم.
        if event_from_user is not None:
            _uname = getattr(event_from_user, "username", None)
            if _uname:
                try:
                    from helpers.redis import cache_username_id
                    asyncio.create_task(cache_username_id(_uname, event_from_user.id))
                except Exception:
                    pass

        # نفس التخزين الانتهازي، لكن أيضاً لصاحب الرسالة المردود عليها
        # (reply_to_message) ولأي Text Mention داخل الرسالة — مصدر غني جداً
        # بمستخدمين لم يكتبوا هم أنفسهم للبوت مباشرة.
        if aio_message is not None:
            try:
                from helpers.redis import cache_username_id as _cache_uid
                reply_user = getattr(getattr(aio_message, "reply_to_message", None), "from_user", None)
                if reply_user is not None and getattr(reply_user, "username", None):
                    asyncio.create_task(_cache_uid(reply_user.username, reply_user.id))
                for _ent in (getattr(aio_message, "entities", None) or []):
                    _ent_user = getattr(_ent, "user", None)
                    if _ent_user is not None and getattr(_ent_user, "username", None):
                        asyncio.create_task(_cache_uid(_ent_user.username, _ent_user.id))
            except Exception:
                pass
