...
AQAB_PUNISHMENTS = [
    ...
    "قول لاي بنت موجود في المجموعة كلمة حلوه",
    "قول قصيدة",
    "سامحتك خلاص مافيه عقاب لك",
    "تاخذ عقابين",
...
# ==== دوال مشتركة: تعطيل/تفعيل لعبة معينة داخل مجموعة محددة (لمالك المجموعة) ====
async def is_game_disabled_in_chat(chat_id, game_name):
    from helpers.context import get_global_r, get_global_dev
    r = get_global_r()
    Dev_FINAL = get_global_dev()
    return bool(await r.get(f'{chat_id}:game_disabled:{game_name}:{Dev_FINAL}'))

async def set_game_disabled_in_chat(chat_id, game_name, disabled: bool):
    from helpers.context import get_global_r, get_global_dev
    r = get_global_r()
    Dev_FINAL = get_global_dev()
    key = f'{chat_id}:game_disabled:{game_name}:{Dev_FINAL}'
    if disabled:
        await r.set(key, '1')
    else:
        await r.delete(key)
